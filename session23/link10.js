...
do {
    ...
    switch (choice) {
        case 1:
            count = prompt("Nhập số phần tử cần nhập: ");
            if (!isNaN(count) < 0) {
            alert("số lượng phần tử không hợp lệ");
            } else {
            for (i = 0; i < count; i++) {
                inputValue = prompt("giá trị các phần tử: ");
        ...
        case 2:
            if(array.length === 0){
                alert("không có giá trị nào trong mảng!")
            } else (alert(array))
            break;
            
        case 3:
            inputValue = ("In ra các phần tử chẵn, tính tổng và sắp xếp giảm dần");
